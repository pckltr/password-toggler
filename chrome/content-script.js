...
// @version      1.0
// @description  Adds buttons to show/hide password in inputs
// @author       YuuMeInc
// @match        *://*/*
// ==/UserScript==
...
(function() {
    ...
    for (var b = 0; b < passwordInputs.length; b++) {
        ...
        passwordInputs[b].parentElement.setAttribute("style", "position: relative; overflow: visible;");
        passwordInputs[b].parentElement.appendChild(button);
        button.setAttribute("style", buttonStyle + backgroundEye);
        button.setAttribute("class", "passwordToggler");
        button.setAttribute("title", "Show password");
    }

    // get all buttons
    var passwordTogglers = document.getElementsByClassName('passwordToggler');
        ...
        for (var i = 0; i < passwordInputs.length; i++) {
            if (passwordInputs[i].type === 'password') {
                ...
                for (var j = 0; j < passwordTogglers.length; j++) {
                    ...
                    passwordTogglers[j].setAttribute("style", buttonStyle + backgroundEyeCut);
                }
            } else {
                passwordInputs[i].type = 'password';
                for (var k = 0; k < passwordTogglers.length; k++) {
                    passwordTogglers[k].title = 'Show password';
                    passwordTogglers[k].setAttribute("style", buttonStyle + backgroundEye);
                }
            }
        }
    }

    // TODO check width and position buttons inside if necessary
