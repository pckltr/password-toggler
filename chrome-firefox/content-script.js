// ==UserScript==
// @name         Password Toggler - view typed passwords
// @version      1.1
// @description  Adds button to show password in web pages
// @author       YuuMeInc
// @match        *://*/*
// ==/UserScript==

(function() {
    'use strict';

    // get all page inputs
    var pageInputs = document.getElementsByTagName("input"),
        passwordInputs = [],
        buttonStyle = "top: 50%; right: 8px; position: absolute; width: 28px; height: 28px; cursor: pointer; transform: translateY(-50%); background-size: cover; z-index: 999999999999999999; background: #dcdcdc; border-radius: 100%; background-repeat: no-repeat; background-position: center center; box-sizing: border-box;",
        backgroundEye = "background-image: url(data:image/svg+xml;base64,);",
        backgroundEyeCut = "background-image: url(data:image/svg+xml;base64,);";

    // add password inputs into an array
    for (var a = 0; a < pageInputs.length; a++) {
        if (pageInputs[a].hasAttribute("type") && pageInputs[a].getAttribute("type") === "password") {
            passwordInputs.push(pageInputs[a]);
        }
    }

    // add buttons to each password input
    for (var b = 0; b < passwordInputs.length; b++) {
        var button = document.createElement("div");
        passwordInputs[b].parentElement.setAttribute("style", "position: relative; overflow: visible;");
        passwordInputs[b].parentElement.appendChild(button);
        button.setAttribute("style", buttonStyle + backgroundEye);
        button.setAttribute("class", "passwordToggler");
        button.setAttribute("title", "Show password");
    }

    // get all buttons
    var passwordTogglers = document.getElementsByClassName('passwordToggler');

    // toggle password inputs title, type and change button style
    function togglePasswords() {
        for (var i = 0; i < passwordInputs.length; i++) {
            if (passwordInputs[i].type === 'password') {
                passwordInputs[i].type = 'text';
                for (var j = 0; j < passwordTogglers.length; j++) {
                    passwordTogglers[j].title = 'Hide password';
                    passwordTogglers[j].setAttribute("style", buttonStyle + backgroundEyeCut);
                }
            } else {
                passwordInputs[i].type = 'password';
                for (var k = 0; k < passwordTogglers.length; k++) {
                    passwordTogglers[k].title = 'Show password';
                    passwordTogglers[k].setAttribute("style", buttonStyle + backgroundEye);
                }
            }
        }
    }

    // add click event to buttons
    (function() {
        for (var i = 0; i < passwordTogglers.length; i++) {
            passwordTogglers[i].addEventListener('click', togglePasswords, false);
        }
    })();
})();
